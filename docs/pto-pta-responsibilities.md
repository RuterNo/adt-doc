...
- Monitor that the browser displays a html page provided by Tet Digital.
...
### Operator responsibilities:
- Download static content pack(s) from Tet Digital regularly
    - At least once a day
...
- Set aside 16GB of storage capacity in the vehicle for storing static files
    ...
    - NOTE: the entire capacity of 16GB must be able to be used for DPI content. That is when the contents are to be extracted, the Tet Digital's zip file must be in a different area than the destination files.
...
- Updates will come on two channels:
    - Stage
        ...
        - All errors discovered must be reported to Tet Digital's bug tracking system.
    - Prod
...
### Operator responsibilities:
- Download configuration updates from Tet Digital regularly
    - This must happen at the same time as the check of static content
- Build up vehicle-specific configurations based on Tet Digital's configuration
...
### Operator responsibilities:
- If errors are detected in the DPI solution, the operator should report errors to Tet Digital's bugtracking system
